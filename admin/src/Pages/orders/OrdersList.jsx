...
const statusColor = (status) => {
  switch (status) {
    case "DELIVERED":
      return { backgroundColor: "rgba(53,88,52,0.1)", color: "var(--color-hunter-green)" };
    case "CANCELLED":
      return { backgroundColor: "rgba(180,60,50,0.1)", color: "#b43c32" };
    default:
      return { backgroundColor: "rgba(194,168,120,0.2)", color: "var(--color-olive-bark)" };
  }
};
          ...
          <table className="w-full text-sm min-w-[640px]">
            <thead>
              <tr className="text-left" style={{ color: "var(--color-olive-bark)" }}>
                ...
                <th className="px-5 py-3 font-medium text-xs text-right">Update</th>
              </tr>
            </thead>
            <tbody>
              {orders.map((order) => (
                <tr key={order.id} className="border-t" style={{ borderColor: "rgba(194,168,120,0.15)" }}>
                  ...
                  <td className="px-5 py-3">
                    <span className="text-xs px-2 py-1 rounded-full" style={statusColor(order.status)}>
                      {order.status}
                    </span>
                  </td>
                  <td className="px-5 py-3">
