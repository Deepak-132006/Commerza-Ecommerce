...
const Home = () => {
  ...
  return (
    <div className="font-sans-body" style={{ backgroundColor: "var(--color-porcelain)" }}>
      <style>{THEME}</style>
      <Navbar />
        ...
        <div className="max-w-6xl mx-auto px-5 py-16 md:py-24 relative z-10">
          ...
          <form onSubmit={handleSearch} className="mt-8 max-w-xl relative">
            <input
              ...
              style={{ backgroundColor: "var(--color-cwhite)", color: "var(--color-evergreen)" }}
            />
            <button
              type="submit"
              aria-label="Search"
        ...
        <div className="max-w-6xl mx-auto px-5 py-6 grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            ...
            <div key={label} className="flex items-center gap-3">
              <Icon size={22} strokeWidth={1.5} style={{ color: "var(--color-hunter-green)" }} />
              <div>
      ...
      <section className="max-w-6xl mx-auto px-5 py-14">
        <div className="flex items-end justify-between mb-6">
          <div>
            <h2 className="font-serif-display text-3xl" style={{ color: "var(--color-evergreen)" }}>
              Top Categories
            </h2>
        ...
        {!catLoading && catError && (
          <div className="flex flex-col items-center text-center py-16">
            <AlertTriangle size={24} strokeWidth={1.5} style={{ color: "var(--color-evergreen)" }} />
            <p className="mt-3 text-sm" style={{ color: "var(--color-olive-bark)" }}>
              {catError}
            </p>
          </div>
        )}
        ...
        {!catLoading && !catError && categories.length === 0 && (
          <div className="flex flex-col items-center text-center py-16">
            <PackageOpen size={24} strokeWidth={1.5} style={{ color: "var(--color-evergreen)" }} />
            <p className="mt-3 text-sm" style={{ color: "var(--color-olive-bark)" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {categories.map((category) => (
              <div
                ...
                tabIndex={0}
                onKeyDown={(e) => {
                  if (e.key === "Enter") navigate(`/products/category/${category.id}`);
                }}
              >
                <img
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                ...
                style={{ backgroundColor: "var(--color-cwhite)", borderColor: "rgba(194,168,120,0.2)" }}
              >
                <div className="h-44 animate-pulse" style={{ backgroundColor: "var(--color-porcelain)" }} />
                <div className="p-4 space-y-3">
                  <div className="h-4 rounded animate-pulse w-3/4" style={{ backgroundColor: "var(--color-porcelain)" }} />
                  <div className="h-4 rounded animate-pulse w-1/2" style={{ backgroundColor: "var(--color-porcelain)" }} />
                </div>
              </div>
            ))}
          </div>
                ...
                >
                  ...
                  >
                    <Heart
                      ...
                      style={!isFav ? { color: "var(--color-olive-bark)" } : undefined}
                    />
                  </button>
                  ...
                  <div className="p-4">
                    ...
                    {product.rating && (
                      <div className="flex items-center gap-1 mt-1.5">
                        <Star size={12} className="fill-[var(--color-soft-fawn)] text-[var(--color-soft-fawn)]" />
                        <span className="text-xs" style={{ color: "var(--color-olive-bark)" }}>
                          {product.rating}
                        </span>
                      </div>
                    )}

                    <p className="text-lg font-semibold mt-2" style={{ color: "var(--color-hunter-green)" }}>
                    ...
                    <div className="flex gap-2 mt-3">
                      ...
                      <button
                        ...
                        style={{ borderColor: "var(--color-hunter-green)", color: "var(--color-hunter-green)" }}
                      >
                        {addingId === product.id ? (
                          "…"
                        ) : (
                          <>
                            <ShoppingCart size={13} className="inline -mt-0.5 mr-1" />
                            Cart
                          </>
                        )}
                      </button>
      ...
      <section style={{ backgroundColor: "var(--color-hunter-green)" }}>
        <div className="max-w-6xl mx-auto px-5 py-14 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-center md:text-left">
            <h2 className="font-serif-display text-2xl md:text-3xl" style={{ color: "var(--color-cwhite)" }}>
              Get 10% off your first order
            </h2>
            <p className="mt-2 text-sm" style={{ color: "rgba(253,253,255,0.75)" }}>
      ...
      <footer style={{ backgroundColor: "var(--color-evergreen)" }}>
        <div className="max-w-6xl mx-auto px-5 py-12 grid grid-cols-2 md:grid-cols-4 gap-8 text-sm">
          <div className="col-span-2 md:col-span-1">
            <p className="font-serif-display text-xl" style={{ color: "var(--color-cwhite)" }}>
              Store
            </p>
          ...
          {[
            ...
            <div key={col.title}>
              <p className="font-medium mb-3" style={{ color: "var(--color-cwhite)" }}>
                {col.title}
              </p>
        ...
        <div
          ...
          style={{ borderColor: "rgba(253,253,255,0.1)", color: "rgba(253,253,255,0.5)" }}
        >
